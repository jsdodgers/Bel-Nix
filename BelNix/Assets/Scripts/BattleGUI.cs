...
public class BattleGUI : MonoBehaviour {
	...
	void setACText() {
		if (mapGenerator.selectedUnit == null) return;
		inventoryAC.text = "AC: " + mapGenerator.selectedUnit.getAC();
	}
	...
	public static void deselectItem() {
		battleGUI.deselectItem((battleGUI.overlayObjects.Count > 0 ? battleGUI.overlayObjects[0] : null));
	}
	...
	public void deselectItem(Image overlayObject) {
		...
		if (UnitGUI.armorSlots.Contains(insertSlot)) {
			...
			mapGenerator.selectedUnit.idleAnimation(false);
			mapGenerator.selectedUnit.Invoke("beginIdle",0.05f);
	...
	public static void selectItem() {
		battleGUI.selectItem((battleGUI.overlayObjects.Count > 0 ? battleGUI.overlayObjects[0] : null));
	}
	...
	public void selectItem(Image overlayObject) {
		...
		if (UnitGUI.armorSlots.Contains(sl)) {
			...
			ItemReturn i2 = mapGenerator.selectedUnit.characterSheet.characterSheet.inventory.removeItemFromSlot(sl);
			i = i2.item;
	...
	//	c.a = 103.0f/255.0f;
		...
		if (selectedItem == null) {
			if (UnitGUI.inventorySlots.Contains(slot)) {
				InventoryItemSlot iis = mapGenerator.selectedUnit.characterSheet.characterSheet.inventory.inventory[(int)slot - (int)InventorySlot.Zero];
				if (iis.hasItem()) {
