...
public class EndGameUnit : MonoBehaviour  {
	...
	//	u.resetAllSprites();
		...
		if (b != null && b.spritePrefab != null) setBoots(b.spritePrefab.GetComponent<SpriteRenderer>().sprite, u.characterSheet.characterSheet.characterColors.secondaryColor);
		else boots.gameObject.SetActive(false);
		Armor p = u.characterSheet.characterSheet.characterLoadout.pantsSlot;
		if (p != null && p.spritePrefab != null) setPants(p.spritePrefab.GetComponent<SpriteRenderer>().sprite, u.characterSheet.characterSheet.characterColors.secondaryColor);
		else pants.gameObject.SetActive(false);
		Armor s = u.characterSheet.characterSheet.characterLoadout.chestSlot;
		if (s != null && s.spritePrefab != null) setShirt(s.spritePrefab.GetComponent<SpriteRenderer>().sprite, u.characterSheet.characterSheet.characterColors.primaryColor);
		else shirt.gameObject.SetActive(false);
		Weapon w = u.getWeapon();
		if (w != null && w.spritePrefab != null) setWeapon(w.spritePrefab.GetComponent<SpriteRenderer>().sprite);
